...
bool BluetoothAddress::GetLocalAddress(uint8_t *local_addr)
{
  ...
  const char *persist_bd_addr_prop = Util::getPersistBdAddrProp();
  ...
  if (!valid_bda) {
    if (strcmp(bt_address_ins.bt_bdaddr_path, "Not set") == 0) {
      if (Logger :: Get()->PropertyGet(Util::getBdAddrProp(), property, NULL)) {
  ...
  if (!valid_bda) {
    if (memcmp(bt_address_ins.factory_bt_bdaddr, zero_bdaddr, kBytes) == 0) {
      if (Logger :: Get()->PropertyGet(Util::getFactoryBdAddrProp(), property, NULL) &&
                   ...
                   property, "false") && (strcmp(property, "true") == 0))
 {
      ...
      while (retry < QMI_INIT_MAX_TRY) {
        ...
        if (QmiDmsInit() == true) {
          if (QmiDmsGetAddress(modem_bdaddr_) == true) {
            ...
            if (Logger :: Get()->PropertySet(persist_bd_addr_prop, (char*)bdstr) < 0) {
                ALOGE("%s: Failed to set MODEM_NV_BD_ADDR in prop %s", __func__,
                    persist_bd_addr_prop);
            }
          }
          QmiDmsCleanup();
          break;
  ...
  if (!valid_bda && Logger :: Get()->PropertyGet(persist_bd_addr_prop, property, NULL) &&
      StringToBytes(property, local_addr)) {
    valid_bda = true;
    le2bd(local_addr);
  }
  ...
  if (!valid_bda) {
    ...
    ALOGE("%s: No preset BDA! Generating BDA: %s for prop %s", __func__,
          (char*)bdstr, persist_bd_addr_prop);
