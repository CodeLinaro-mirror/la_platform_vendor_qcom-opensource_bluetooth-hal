...
void Logger :: DecodeHwErrorEvt(uint8_t *buff)
{
  ...
  ALOGD("%s mini_dump_size = %zu", __func__, mini_dump_size_);
...
void Logger::FrameBqrRieEvent(hidl_vec<uint8_t>*bt_packet_)
{
  ...
  if (pri_crash_reason == BT_HOST_REASON_SOC_CRASHED
    ...
    if (hci_buffer_dump_size_ and remaining_bytes_for_vs_params > BQR_RIE_VSP_META_DATA_SIZE) {
      ...
      ss << "\n";
      ALOGD("%s : Sending hci buffer dump of size %zu along with BQR RIE", __func__,
            hci_buffer_dump_size_);
      ...
      ss << "\n";
      ALOGD("%s : Sending minidump of size %zu along with BQR RIE", __func__,
            mini_dump_size_);
    }
  }
...
void Logger :: Init(HciTransport *hci_transport)
{
  ...
  if (status) {
    ...
    if (Util::isDiagEnabled()) {
      ALOGD("%s: Registered Diag callbacks for BT%d", __func__, Util::getHwSlot());
...
{
  snprintf(path, SOC_DUMP_PATH_BUF_SIZE, SOC_DUMP_PATH,
          Util::getString(SOC_DUMP_PREFIX, NEW_SOC_DUMP_PREFIX),
          time_year, time_month, time_day, time_hour, time_min, time_sec);
...
void Logger :: GetUartDumpFilename(short int uart_dump, char* path)
{
  switch (uart_dump){
  case IpcLogs::TX_LOG:
    snprintf(path, UART_IPC_PATH_BUF_SIZE, UART_IPC_TX_LOGS_DEST_PATH,
             Util::getString(UART_IPC_TX_LOG_PREFIX, NEW_UART_IPC_TX_LOG_PREFIX),
             time_year, time_month, time_day, time_hour, time_min, time_sec);
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::RX_LOG:
    snprintf(path, UART_IPC_PATH_BUF_SIZE, UART_IPC_RX_LOGS_DEST_PATH,
             Util::getString(UART_IPC_RX_LOG_PREFIX, NEW_UART_IPC_RX_LOG_PREFIX),
             time_year, time_month, time_day, time_hour, time_min, time_sec);
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::STATE_LOG:
    snprintf(path, UART_IPC_PATH_BUF_SIZE, UART_IPC_STATE_LOGS_DEST_PATH,
             Util::getString(UART_IPC_STATE_LOG_PREFIX, NEW_UART_IPC_STATE_LOG_PREFIX),
             time_year, time_month, time_day, time_hour, time_min, time_sec);
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::POWER_LOG:
    snprintf(path, UART_IPC_PATH_BUF_SIZE, UART_IPC_PWR_LOGS_DEST_PATH,
             Util::getString(UART_IPC_PWR_LOG_PREFIX, NEW_UART_IPC_PWR_LOG_PREFIX),
             time_year, time_month, time_day, time_hour, time_min, time_sec);
    dump_files.push_back(std::string(path));
    break;
  }
...
void Logger::GetUartLogFilename(char* path) {
  snprintf(path, UART_LOG_PATH_BUF_SIZE, UART_LOGS_DEST_PATH,
           Util::getString(UART_LOG_PREFIX, NEW_UART_LOG_PREFIX),
           time_year, time_month, time_day, time_hour, time_min, time_sec);
  dump_files.push_back(std::string(path));
}
...
{
  snprintf(path, BT_FILE_NAME_LEN, SNOOP_FILE_PATH,
           Util::getString(SNOOP_FILE_NAME_PREFIX, NEW_SNOOP_FILE_NAME_PREFIX),
           time_year, time_month, time_day, time_hour, time_min, time_sec);
  dump_files.push_back(std::string(path));
}
#endif
...
{
  snprintf(path, BT_FILE_NAME_LEN, BT_STATE_FILE_PATH,
           Util::getString(BT_STATE_FILE_NAME_PREFIX, NEW_BT_STATE_FILE_NAME_PREFIX),
           time_year, time_month, time_day, time_hour, time_min, time_sec);
  dump_files.push_back(std::string(path));
}
...
void Logger::DumpLogsInBugreport(int fd)
{
  ...
  current_file = LastestLogFileName(Util::getString(BT_STATE_FILE_NAME_PREFIX, NEW_BT_STATE_FILE_NAME_PREFIX), LOG_COLLECTION_DIR);
  ...
  current_file = LastestLogFileName(Util::getString(SNOOP_FILE_NAME_PREFIX, NEW_SNOOP_FILE_NAME_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(UART_IPC_PWR_LOG_PREFIX, NEW_UART_IPC_PWR_LOG_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(UART_IPC_STATE_LOG_PREFIX, NEW_UART_IPC_STATE_LOG_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(UART_IPC_RX_LOG_PREFIX, NEW_UART_IPC_RX_LOG_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(UART_IPC_TX_LOG_PREFIX, NEW_UART_IPC_TX_LOG_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(UART_LOG_PREFIX, NEW_UART_LOG_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  current_file = LastestLogFileName(Util::getString(SOC_DUMP_PREFIX, NEW_SOC_DUMP_PREFIX), LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  any_reg_file = false;
  WriteToFd(fd, latest_file.ts, Util::getString(BT_STATE_FILE_NAME_PREFIX, NEW_BT_STATE_FILE_NAME_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(SNOOP_FILE_NAME_PREFIX, NEW_SNOOP_FILE_NAME_PREFIX), LOG_COLLECTION_DIR, SNOOP_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(UART_IPC_PWR_LOG_PREFIX, NEW_UART_IPC_PWR_LOG_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(UART_IPC_STATE_LOG_PREFIX, NEW_UART_IPC_STATE_LOG_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(UART_IPC_RX_LOG_PREFIX, NEW_UART_IPC_RX_LOG_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(UART_IPC_TX_LOG_PREFIX, NEW_UART_IPC_TX_LOG_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(UART_LOG_PREFIX, NEW_UART_LOG_PREFIX), LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, Util::getString(SOC_DUMP_PREFIX, NEW_SOC_DUMP_PREFIX), LOG_COLLECTION_DIR, FW_DUMP_EXTN);
}
