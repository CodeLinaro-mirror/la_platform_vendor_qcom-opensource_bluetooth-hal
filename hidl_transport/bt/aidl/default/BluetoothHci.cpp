...
{
  BluetoothHci(BLUETOOTH_SLOT_0);
}

BluetoothHci::BluetoothHci(int slot)
...
{
  ALOGW("BluetoothHci::close(), slot%d", hw_slot_);
  if (deathRecipient != nullptr) deathRecipient->UnlinkToDeath(event_cb_);
  DataHandler::CleanUp(TYPE_BT);
  ALOGW("BluetoothHci::close, finish cleanup slot%d", hw_slot_);
  return ndk::ScopedAStatus::ok();
}
